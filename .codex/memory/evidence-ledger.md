...
| 2026-07-19 | One-click local Windows source automation | `Build-Windows.cmd`/`bin/Build-Windows.ps1` parsed successfully; default bootstrap is isolated VS 2022/Cygwin, one hidden UAC child, LF snapshot, native target sequence, and final-MSI extraction. The current host has VS 2026 rather than the dedicated VS 2022 instance and no isolated Cygwin profile; no bootstrap was run | Source automation only; no native build, installer, runtime interaction, or visual evidence |
