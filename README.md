...
| Canonical rewrite coverage | Contracted; implementation in progress | The operator-provided design ZIP is pinned by hash in the [`Windows rewrite contract`](docs/design/00-windows-rewrite-contract.md). Its eleven surfaces govern the native Windows rewrite. Fail-closed registries track all 599 top-level LibreOffice dialog roots and 26 audited shipping text-query fields plus one planned Start Center search. Current native source adds one centralized Windows-only final-layout hook that bottom-right positions `Dialog` instances inside the visible owner/work area; this is placement infrastructure, not the customizable form host, manager/history, regex integration, build, or runtime completion |
...
notification persistence, customization, stacking, bulk management, Git-backed
undo, or compiled/headless proof.
