...
"""Fail-closed source contract for the Material Calc sheet-tab strip (ScTabControl).

``qa/windows-ui-contract/calc-sheet-tabs.json`` registers the Calc sheet-tab
surface from docs/design/05-navigation.md 6 and 06-containers.md 6.3. The strip
band, docked-tab fills and Add-Sheet button are drawn by the svtools TabBar /
TabDrawer base class (out of this row's file scope); this checker cross-validates
what ``ScTabControl`` owns natively and additively against the real tree:

* ``definition_parts`` -- the tabitem / tabheader / tabpane parts the sheet-tab
  strip maps its tokens onto must exist in
...
* ``paint_layout`` -- the ``ScTabControl::Paint`` override must call the base
  ``TabBar::Paint`` (never replace it) and carry every layout marker, and the
  header must declare the override plus the Material members/helpers.
* ``color_strip_independence`` -- the accent-strip overlay body must draw from the
  stored colour map and must NOT consult ``IsPageSelected`` / ``GetCurPageId``, so
  a user tab colour is rendered independently of the selection treatment (the
  accessibility rule of docs/design/05-navigation.md 6.4).
...
REGISTRY_PATH = "qa/windows-ui-contract/calc-sheet-tabs.json"
DEFINITION_PATH = "vcl/uiconfig/theme_definitions/material/definition.xml"

REQUIRED_SURFACE_IDS = {"calc.sheet-tabs"}
    ...
    for surface in registry.get("surfaces", []):
        ...
        for owner in surface.get("owner_sources", []):
            if isinstance(owner, str):
                paths.add(owner)
        for key in ("guarded_token_consumption", "paint_layout", "color_strip_independence"):
...
def _validate_color_strip_independence(
    context: str, block: Mapping[str, Any], contents: Mapping[str, str], errors: list[str]
) -> None:
    source_path = block.get("source")
    method = block.get("function")
    source = contents.get(source_path) if isinstance(source_path, str) else None
    if source is None:
        errors.append(f"{context}:color-strip:source {source_path} missing")
        return
    if not isinstance(method, str):
        errors.append(f"{context}:color-strip:function must be a string")
        return
    body = _function_body(_without_cpp_comments(source), method + "(")
    if body is None:
        errors.append(f"{context}:color-strip:function {method} not found")
        return
    for marker in block.get("must_contain", []):
        if isinstance(marker, str) and marker not in body:
            errors.append(f"{context}:color-strip:{method} must draw from the colour map ({marker})")
    for marker in block.get("must_not_contain", []):
        if isinstance(marker, str) and marker in body:
            errors.append(
                f"{context}:color-strip:{method} must not consult the selection state "
                f"(found {marker}); the accent must stay independent of it"
            )
...
def violations(registry: Mapping[str, Any], contents: Mapping[str, str]) -> list[str]:
    ...
    if registry.get("contract") != "material-calc-sheet-tabs":
    ...
    for index, surface in enumerate(surfaces):
        ...
        strip_block = surface.get("color_strip_independence")
        if isinstance(strip_block, dict):
            _validate_color_strip_independence(context, strip_block, contents, errors)
        else:
            errors.append(f"{context}:color_strip_independence:object required")
...
def main() -> int:
    try:
        ...
        print(f"Calc sheet-tab contract failed:\n{error}", file=sys.stderr)
        return 1
    registry, _ = load_repository()
    print(
        "Calc sheet-tab contract passed: "
        f"{len(registry['surfaces'])} ScTabControl surface(s) with tabitem token fidelity, "
        "guarded token consumption, additive Paint override and selection-independent "
        "colour-strip accent"
    )
    return 0
