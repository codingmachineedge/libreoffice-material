...
"""Fail-closed source contract for the Material Calc classic chrome (WIN-CA-001).

``qa/windows-ui-contract/calc-chrome.json`` pins the Calc classic-chrome
composition from docs/design/10-writer-calc.md (10.1 shared chrome, 10.3 Calc).
Calc toolbars and the menu bar are data-driven uiconfig XML, so the M-scope of
...
native toolbar part contract, never re-drawing controls. This checker parses the
real tree fail-closed and cross-validates every declaration:

* ``chrome_parts`` -- the native ``toolbar`` band / separator / nine-state
  ``Button`` parts that every Calc chrome control resolves through must exist in
  ...
  dark palettes. A renamed part, dropped state, or token drift fails closed. The
  definition file is read only, never mutated.
* ``toolbars`` -- the FMT.calc formatting toolbar
  (sc/uiconfig/scalc/toolbar/formatobjectbar.xml) and the standard toolbar
...
REGISTRY_PATH = "qa/windows-ui-contract/calc-chrome.json"
...
REQUIRED_TOOLBAR_IDS = {"FMT.calc", "standard"}
...
def violations(registry: Mapping[str, Any], contents: Mapping[str, str]) -> list[str]:
    ...
    if registry.get("contract") != "material-calc-chrome-composition":
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Calc chrome composition contract failed:\n{error}", file=sys.stderr)
        return 1
    registry, _ = load_repository(repo_root)
    print(
        "Calc chrome composition contract passed: "
        f"{len(registry['toolbars'])} pinned toolbar(s) plus the menu bar, the native "
