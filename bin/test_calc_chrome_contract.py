...
"""Mutation regressions for the Material Calc classic chrome contract."""
...
VALIDATOR_PATH = REPOSITORY / "bin/check-calc-chrome-contract.py"
SPEC = importlib.util.spec_from_file_location("check_calc_chrome_contract", VALIDATOR_PATH)
...
FMT = "sc/uiconfig/scalc/toolbar/formatobjectbar.xml"
STD = "sc/uiconfig/scalc/toolbar/standardbar.xml"
MENU = "sc/uiconfig/scalc/menubar/menubar.xml"


class CalcChromeContractTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_preserved_expert_command_removed_fails(self) -> None:
        ...
        source = self.contents[FMT].replace(
            'xlink:href=".uno:NumberFormatStandard"', 'xlink:href=".uno:NumberFormatStandardX"', 1
        )
        self.assert_mutation_changed(FMT, source)
        errors = self.failures(contents=self.with_content(FMT, source))
        self.assertTrue(
    ...
    def test_missing_required_toolbar_fails(self) -> None:
        ...
        errors = self.failures(registry=registry)
        self.assertTrue(any("missing required FMT.calc" in e for e in errors), errors)
