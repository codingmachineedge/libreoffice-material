...
"""Fail-closed source contract for the Material Writer classic chrome (WIN-WR-001).

``qa/windows-ui-contract/writer-chrome.json`` pins the Writer classic-chrome
composition from docs/design/10-writer-calc.md (10.1 shared chrome, 10.2 Writer).
Writer toolbars and the menu bar are data-driven uiconfig XML, so the M-scope of
this row is *pinning composition* -- button command identity + order, separator
placement, per-button visibility -- and grounding the Material treatment in the
native toolbar part contract, never re-drawing controls. The ribbon/notebookbar
restyle is prototype-only (not yet in native source per design 10.1), so it has
no source to pin and is deliberately out of this contract.

This checker parses the real tree fail-closed and cross-validates every
declaration:

* ``chrome_parts`` -- the native ``toolbar`` band / separator / nine-state
  ``Button`` parts that every Writer chrome control resolves through must exist
  in vcl/uiconfig/theme_definitions/material/definition.xml with the exact
  fill / stroke / stroke-width / radius tokens (the Button at
  ``@corner-toolbar``), every declared metric must carry its exact value, and
  every palette role the states reference must resolve in *both* the light and
  dark palettes. This is the same shared, app-agnostic VCL part contract that
  WIN-CA-001 pins -- it is not Calc-specific. A renamed part, dropped state, or
  token drift fails closed. The definition file is read only, never mutated.
* ``toolbars`` -- the FMT.writer formatting toolbar
  (sw/uiconfig/swriter/toolbar/textobjectbar.xml) and the standard toolbar
  (standardbar.xml) are parsed and their *entire* ordered composition
  (toolbaritem command identity + visibility and every separator, in order) must
  match the pinned sequence exactly. A reorder, removed/added item, flipped
  visibility, or moved separator fails closed. Each toolbar's ``design_core``
  commands must be present and visible (the design-chapter primary set) and its
  ``preserved_commands`` must remain present at any visibility (the expert
  commands the redesign must never rebind or remove).
* ``menu`` -- the menu bar top-level ``menu:menu`` id sequence must match
  exactly and every ``design_core`` menu must be present. Writer's top level
  swaps Calc's SheetMenu/DataMenu for TableMenu/FormatFormMenu.
* ``density`` / ``combo_annotations`` -- honest carve-out guards: density
  selection and overflow collapse, and the prototype combo widths, are
  specified-only (build-dependent), so their ``status`` must stay ``specified``
  and is never promoted to an implemented claim.

It is source evidence only: ``runtime_verified`` is false throughout -- no native
build, toolbar pixels, or runtime interaction are claimed.
"""
...
REGISTRY_PATH = "qa/windows-ui-contract/writer-chrome.json"
...
REQUIRED_TOOLBAR_IDS = {"FMT.writer", "standard"}
...
def _validate_simple_part(
    ...
    """A single-``<state>`` part (band / entire / separator): match the attribute-free
    state and cross-check its declared drawing element and tokens."""
...
def _validate_toolbar(
    ...
    # Exact ordered composition: identity + visibility + separator placement.
...
def _validate_carveout(name: str, block: Mapping[str, Any], errors: list[str]) -> None:
    if block.get("status") != "specified":
        errors.append(
            f"{name}:status:must stay 'specified' (density selection / overflow / combo "
            "widths are build-dependent and must not be promoted to an implemented claim)"
        )
...
def violations(registry: Mapping[str, Any], contents: Mapping[str, str]) -> list[str]:
    ...
    if registry.get("contract") != "material-writer-chrome-composition":
    ...
    missing_required = REQUIRED_TOOLBAR_IDS - seen_ids
    if missing_required:
        errors.append(f"registry:toolbars:missing required {', '.join(sorted(missing_required))}")

    menu = registry.get("menu")
    if isinstance(menu, dict):
        menu_file = menu.get("file")
        menu_root = _parse_xml(contents.get(menu_file) if isinstance(menu_file, str) else None, "menu", errors)
        if menu_root is not None:
            _validate_menu(menu_root, menu, errors)
    else:
        errors.append("registry:menu:object required")

    for name in ("density", "combo_annotations"):
        block = registry.get(name)
        if isinstance(block, dict):
            _validate_carveout(name, block, errors)
        else:
            errors.append(f"registry:{name}:object required")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Writer chrome composition contract failed:\n{error}", file=sys.stderr)
        return 1
    registry, _ = load_repository(repo_root)
    print(
        "Writer chrome composition contract passed: "
        f"{len(registry['toolbars'])} pinned toolbar(s) plus the menu bar, the shared native "
        "toolbar band/separator/nine-state Button parts at @corner-toolbar, design-core "
        "visibility and preserved expert commands, with density/overflow carved out spec-only."
    )
    return 0
